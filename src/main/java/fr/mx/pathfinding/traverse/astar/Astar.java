...
public class Astar<S> {
  ...
  public Astar(Explorable<S> explorable, S startStep, S endStep, AStarHeuristic<S> heuristic) {
    ...
    while (!openSet.isEmpty()) {
      ...
      for (S step : explorable.getReachableStepsFrom(startStep)) {
        ...
        //        double gScore = current.getgScore() + heuristic.getCostEstimate(current.step,
        // step);
  ...
  static class NodeData<S> implements TracableStep<S> {
    ...
    NodeData(S fromStep, double heuristic, double gScore) {
