...
public class AStarManhattanHeuristic implements AStarHeuristic<Step2D<MapMatrix.Values>> {

  @Override
  public double getCostEstimate(Step2D<MapMatrix.Values> sourceStep, Step2D<MapMatrix.Values> targetStep) {
    return Coords2D.manhattanDistance(new Coords2D(sourceStep.getX(), sourceStep.getY()), new Coords2D(targetStep.getX(), targetStep.getY()));
  }
}
